#!/usr/bin/env python3
"""
Time Crytal Tensor Network Main File

This script creates both:
1. Phase diagram showing DTC stability regions across parameter space
2. Individual figures A-D with Fourier spectra analysis:
   ...
   - Figure C: DTC Decay Dynamics - Open system dephasing effects + spectrum
...
def stringent_dtc_detection(loschmidt_echoes: List[float], times: List[float], 
    ...
    """
    Stringent detection of discrete time crystal behavior using multiple criteria.
    
    Analyzes time series data to identify DTC signatures including:
    1. Period-doubling autocorrelation
    2. Subharmonic spectral peaks
...
def simulate_perfect_dtc(params: Dict) -> Tuple[List[float], List[float], List[float]]:
    ...
    Parameters Used:
        ...
        - χ_max = 256: High bond dimension for accuracy
