...
class TestDTCPhysics(unittest.TestCase):
    ...
    def test_clean_system_behavior(self):
        """Test behavior without disorder (clean limit)."""
        psi_initial = create_initial_state(8, "neel")
        n_periods = 20
        
        # Evolve clean system
        states, times = self.clean_model.evolve(psi_initial, n_periods)
        
        # Calculate observables
        loschmidt_echoes = [calculate_loschmidt_echo(psi_initial, psi) for psi in states]
        
        # Clean system might thermalize - check for reasonable behavior
        final_le = loschmidt_echoes[-1]
        self.assertGreaterEqual(final_le, 0.0, "Loschmidt echo should be non-negative")
        self.assertLessEqual(final_le, 1.0, "Loschmidt echo should not exceed 1")
...
class TestTensorNetworkProperties(unittest.TestCase):
    ...
    def test_bond_dimension_growth(self):
        """Test entanglement growth and bond dimension scaling."""
        psi_initial = create_initial_state(12, "neel")
        n_periods = 15
        
        trunc_params = {'chi_max': 64, 'svd_min': 1e-12, 'trunc_cut': 1e-8}
        floquet_evo = CustomFloquet(self.model, trunc_params)
        
        states, times, info = floquet_evo.evolve_floquet(psi_initial, n_periods)
        
        bond_dims = info['bond_dimensions']
        
        # Initial product state should have bond dimension 1
        self.assertEqual(bond_dims[0], 1, "Initial product state should have χ=1")
        
        # Bond dimension should grow with time (entanglement spreading)
        self.assertGreater(bond_dims[-1], bond_dims[0], 
                          "Bond dimension should grow due to entanglement")
        
        # Should not exceed maximum allowed
        self.assertLessEqual(max(bond_dims), trunc_params['chi_max'],
                            "Bond dimension should not exceed chi_max")
        
    def test_entanglement_spreading(self):
        """Test entanglement spreading in time."""
        psi_initial = create_initial_state(10, "neel")
        
        # Short evolution to see initial spreading
        psi_evolved = self.model.floquet_step(psi_initial)
        
        # Calculate entanglement entropy across different cuts
        ee_initial = []
        ee_evolved = []
        
        for cut in range(1, 9):  # Cuts 1 through 8
            ee_i = psi_initial.entanglement_entropy()[cut]
            ee_e = psi_evolved.entanglement_entropy()[cut]
            ee_initial.append(ee_i)
            ee_evolved.append(ee_e)
        
        # Product state should have zero entanglement
        for ee in ee_initial:
            self.assertAlmostEqual(ee, 0.0, places=10,
                                  msg="Initial product state should have zero entanglement")
        
        # After evolution, should have some entanglement near the center
        center_ee = ee_evolved[len(ee_evolved)//2]
        self.assertGreater(center_ee, 0.0,
                          "Should have entanglement after evolution")
        
    def test_truncation_effects(self):
        """Test effects of truncation on dynamics."""
        psi_initial = create_initial_state(8, "neel")
        n_periods = 10
        ...
        # Higher bond dimension should give more accurate results
        # (though this is not always monotonic due to truncation)
        self.assertTrue(all(overlap >= 0.0 for overlap in final_overlaps),
                       "All overlaps should be non-negative")
        self.assertTrue(all(overlap <= 1.0 for overlap in final_overlaps),
                       "All overlaps should not exceed 1")
